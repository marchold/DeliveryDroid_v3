...
//  DeliveryDroid
//
//  Created by Marc Kluver on 12/7/18.
...
class DataBase {
    ...
    static let PayedSplit      = "PayedSplit"
    static let DeliveryOrder   = "DeliveryOrder"
    static let Shift           = "Shift"
    static let PaymentType     = "PaymentType"
    static let PaymentType2    = "PaymentType2"
    static let ArivalTime      = "ArivialTime"
    static let PaymentTime     = "PaymentTime"
    static let RunNumber       = "RunNumber"
    static let OutOfTown       = "OutOfTown"
    static let OnHold          = "OnHold"
    static let OutOfTown2      = "OutOfTown2"
    static let OutOfTown3      = "OutOfTown3"
    static let OutOfTown4      = "OutOfTown4"
    static let AptNumber       = "AptNumber"
    static let geocodeFailed   = "geocodeFailed"
    static let smsCoustomer    = "smsCoustomer"
    static let manualGPS       = "manualGPS"
    let ODO_START       = "OdomoterStart"
    let ODO_END         = "OdomoterEnd"
    let TIME_START      = "TimeStart"
    let TIME_END        = "TimeEnd"
    static let StartsNewRun    = "StartsNewRun"
    let PAY_RATE        = "PAY_RATE"
    let PAY_RATE_ON_RUN = "PAY_RATE_ON_RUN"

    //New for version 8 of the dataBase
    static let OrderNotes       = "OrderNotes" //Private notes vs. Notes which is for all addresses
    static let ExtraPay         = "ExtraPay"   //Field for extra pay, like for sodas or weird mileage pay
    static let PickUpAddress    = "PickUpAddress" //Some users pick up orders at various stores
    static let CuponDiscount    = "CuponDiscount"

    static let PhoneNumber      = "PhoneNumber"

    static let DATABASE_NAME    = "BuildConfig.DATABASE_NAME" //TODO: build variants with db name
    static let DATABASE_TABLE   = "orders"
    static let DATABASE_VERSION = 11
    
    var TodaysShiftCount = -1
    
    func dataDidJustChange(){
        NotificationCenter
            .default
            .post(name: Notification.Name("com.catglo.deliverydroid.DBCHANGED"),
                                        object: nil)
    }

    var justCreated = false
    
    let DATABASE_CREATE:String =
    ...
    let lock = DispatchQueue(label: "com.test.mySerialQueue")
    ...
    init() {
        lock.sync {
            let fileURL = try! FileManager.default.url(for: .documentDirectory,
                                                       ...
                                                       appropriateFor: nil,
                                                       create: false)
                .appendingPathComponent(DataBase.DATABASE_NAME)
            if (sqlite3_open(fileURL.path, &db) == SQLITE_OK && db != nil) {
                onCreate(db!)
            }
            else {
                //TODO:Deal with this unlikley error print("error opening database")
            }
        }
    }
    
    func execSQL(_ sqlString:String)
    {
    ...
    func onCreate(_ database:OpaquePointer)
    {
        execSQL(DATABASE_CREATE)
        
        execSQL("CREATE TABLE IF NOT EXISTS shifts (ID integer primary key autoincrement,"
                + ODO_START + "  INTEGER, "
                + ODO_END + "    INTEGER, "
                + PAY_RATE + "   FLOAT,"
        ...
        execSQL("CREATE TABLE IF NOT EXISTS dropOffs (ID integer primary key autoincrement,"
       ...
       execSQL("CREATE TABLE IF NOT EXISTS streetNames (ID integer primary key autoincrement,"
                + "lastModificationTime  TIMESTAMP, "
                + "streetName            VARCHAR);")
            
      execSQL("CREATE TABLE IF NOT EXISTS expenses (ID integer primary key autoincrement,"
                + "description       VARCHAR, "
                + "category         VARCHAR, "
                + "amount           FLOAT, "
      ...
      execSQL("CREATE TABLE IF NOT EXISTS gps_notes (ID integer primary key autoincrement,"
                + "note                 VARCHAR, "
                + "GPSLat               FLOAT, "
                + "GPSLng               FLOAT, "
                + "orderId              INT,"
     ...
     execSQL("CREATE TABLE IF NOT EXISTS hours_worked (ID integer primary key autoincrement,"
                + "start            TIMESTAMP, "
                + "end              TIMESTAMP, "
                + "rate             FLOAT, "
                + "lastModificationTime              TIMESTAMP, "
   ...
   /* execSQL(createTimestampTriggerSql("orders","ID"));
    execSQL(createTimestampTriggerSql("shifts","ID"));
    execSQL(createTimestampTriggerSql("dropOffs","ID"));
    execSQL(createTimestampTriggerSql("streetNames","ID"));
    execSQL(createTimestampTriggerSql("expenses","ID"));
    database.execSQL(createTimestampTriggerSql("gps_notes","ID"));
    database.execSQL(createTimestampTriggerSql("hours_worked","ID"));
    */
        
        //TOOD:database.setVersion(DATABASE_VERSION);
     ...
     */
     ...
     var orderCosts : [String]? = nil
     var orderNumbers : [String]? = nil
     var orderNumberPrefix : String? = nil
    ...
    func add(_ order: Order ) -> Int64 {
        return add(order: order, shiftId: TodaysShiftCount)
    }
    
    func add(order:Order, shiftId:Int) -> Int64 { lock.sync
    {
        //BackupManager.dataChanged(context.getPackageName());
     
        var orderOfNextSmallest : Float = -1
     //long timeOfNextSmallest = Long.MAX_VALUE;
     
        var orderOfNextBiggest : Float = -1
    // long timeOfNextBiggest = Long.MIN_VALUE;
     
        var orderOfSmallest : Float = -1
     //long timeOfSmallest = Long.MAX_VALUE;
     
        var orderOfBiggest : Float = -1
     //long timeOfBiggest = Long.MIN_VALUE;
     
        var myListOrder : Float = -1
