...
//		args.put(PAY_RATE_ON_RUN, shift.payRateOnRun);
    ...
    public TipTotalData getTipTotal(final Context context, String where,String shiftWhere){synchronized (DataBase.class){
		...
		if (c.moveToFirst()) {
			...
			for (WageZone wz : filteredZones){
				...
				TipTotalData.PayRatePieriod payRatePeriod = ret.payRatePieriods.get(payRateStringKey);
	...
	public String getCSVData(Calendar startDate, Calendar endDate,ProgressDialog progressDialog, Activity activity) {synchronized (DataBase.class){
		String query = ""
				+"SELECT `Time`"        //0
				+	   ",`OrderNumber`" //1
				+	   ",`Shift`"       //2
		...
		if (c != null && c.moveToFirst()) {
			...
			do {
                ...
                Log.i("CSV","Processed "+index+" out of "+count+" orders ");
                ...
                boolean isUndeliverable = c.getInt(undeliverableCol)==1;
		...
		if (c != null){
			if (c.moveToFirst()){
				do {
					...
					try {
						wage = Tools.getFormattedCurrency(Tools.parseCurrency(wage));
					} catch (Exception e){
						e.printStackTrace();
					}	
