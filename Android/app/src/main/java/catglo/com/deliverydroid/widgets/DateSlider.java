...
import catglo.com.deliverydroid.widgets.TimeView.TimeTextView;
...
//	private static String TAG = "DATESLIDER";
	...
	protected MutableDateTime mTime;
	protected TextView mTitleText;
	protected List<ScrollLayout> mScrollerList = new ArrayList<ScrollLayout>();
	protected LinearLayout mLayout;
	
	public DateSlider(Context context, OnDateSetListener l, MutableDateTime calendar) {
		super(context);
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (savedInstanceState!=null) {
			long time = savedInstanceState.getLong("time", mTime.getMillis());
			mTime.setMillis(time);
		}
		
		this.requestWindowFeature(Window.FEATURE_CUSTOM_TITLE);
		setContentView(R.layout.dateslider);
		this.getWindow().setFeatureInt(Window.FEATURE_CUSTOM_TITLE, R.layout.date_slider_dialogtitile);
	...
	public Bundle onSaveInstanceState() {
		...
		savedInstanceState.putLong("time", mTime.getMillis());
	...
	protected void arrangeScroller(ScrollLayout source) {
		...
		if (source!=null) {
			for (ScrollLayout scroller: mScrollerList) {
				...
				scroller.setTime(mTime.getMillis(),0);
			}
		}
	}
